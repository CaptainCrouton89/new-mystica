...
struct EnemyDialogueBubble: View {
            ...
            if isVisible {
                ...
                .background(
                    ZStack(alignment: .bottom) {
                        ...
                        // Triangle tail - positioned below the bubble pointing down
                        VStack(spacing: 0) {
                            Spacer()
                            Triangle()
                                .fill(toneColor())
                                .frame(width: 16, height: 10)
                                .offset(y: 5) // Slight overlap for visual continuity
                        }
                    }
                )
                .padding(.bottom, 8) // Space for triangle to be visible
    ...
    #Preview("Confident Tone") {
        StatefulPreviewWrapper(isVisible: true) { binding in
            EnemyDialogueBubble(
...
private struct StatefulPreviewWrapper<Value, Content: View>: View {
    ...
    init(wrappedValue: Value, @ViewBuilder content: @escaping (Binding<Value>) -> Content) {
